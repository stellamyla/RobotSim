...
struct JointCommandInterface: public CommandInterface {
	...
	virtual string MouseInputEvent(int mx, int my, bool drag) {
		if (drag) {
			...
			Config q;
			sim->odesim.robot(0)->GetConfig(q);
			Robot* robot = GetRobot();
			robot->UpdateConfig(q);
			int link;
			Vector3 localPos;
...
struct IKCommandInterface: public CommandInterface {
  ...
  void SetDestination(int link,const Vector3& localPt,const Vector3& dest)
  {
    currentLink = link;
    currentPoint = localPt;
		...
		if (drag) {
			if (currentLink >= 0) {
			  ...
			  Timer timer;
				...
				SetDestination(currentLink,currentPoint,currentDestination);
#if DO_TIMING
			timingStats.collect(timer.ElapsedTime());
#endif
			}
			stringstream ss;
			ss << "Drag " << currentLink << " " << currentPoint << " " << mx
					<< " " << my << endl;
			return ss.str();
		} else {
			Ray3D ray;
...
struct PFCommandInterface: public IKCommandInterface {
	...
	PFCommandInterface() :
		hasGoal(false), initialedMesh(false),
				firstMove(true) {
	}
	virtual string Name() const {
		return "PFPointPoser";
...
struct RealTimePlannerData
{
  ...
  bool reset;              //(in) a new point/destination is set
...
void* planner_thread_func(void * ptr)
{
  ...
  while (true) {
    ...
    bool start = (data->planState==1 && data->currentLink >= 0 && data->planner);
    if(start) {
      data->planState = 2;
    ...
    if(data->reset) {
      data->reset = false;
      IKGoal goal;
      goal.link = data->currentLink;
      goal.localPosition = data->currentPoint;
      goal.SetFixedPosition(data->currentDestination);
      
      if(data->planner) {
	CartesianObjective* obj=new CartesianObjective(data->planner->robot);
	obj->ikGoal = goal;
...
public:
  ...
  MTPlannerCommandInterface() :
    planner(NULL), unsuccessfulPlanTime(0)
  {
  ...
  virtual void DrawGL() {
    if (planner && currentLink >= 0) {
      glPointSize(5.0);
      glEnable( GL_POINT_SMOOTH);
      glDisable( GL_LIGHTING);
      glBegin( GL_POINTS);
      glColor3f(0, 1, 1);
      glVertex3v(GetRobot()->links[currentLink].T_World
		 * currentPoint);
      glColor3f(0, 1, 0.5);
      glVertex3v(currentDestination);
      glEnd();
  ...
  void SetDestination(int link,const Vector3& localPt,const Vector3& dest)
  {
    ...
    pthread_mutex_lock(&data.mutex);
    data.currentLink = currentLink;
    data.currentPoint = currentPoint;
    data.currentDestination = currentDestination;
  ...
  */
	...
	virtual bool Initialize() {
	  realClock.Reset();
		...
		uis[3] = new MTIKPlannerCommandInterface;
		uis[4] = new MTRRTCommandInterface;
	...
	virtual void RenderWorld() {
		...
		*/
		sim.odesim.robot(0)->GetConfig(robot->q);
		robot->UpdateFrames();

		//draw current commanded configuration -- transparent green
		//vector<GLColor> oldColors = world->robots[0].view.colors;
		//GLColor newColor(0, 1, 0, 0.5);
		//world->robots[0].view.SetColors(newColor);
		//KH: this is the wrong  configuration to be testing completion!
		//robot->UpdateConfig(c->xcur);
		...
		if (!simulate) {
			...
			if(scenarioNumber == 0 && currentStep <=4) {
				...
				case 4:
					...
					glutBitmapString(fontface,line.c_str());					
					if (realClock.ElapsedTime()-tutorialFinishTime>5.0) {
						currentStep++;
					}
				}
			}else if(!finished) {
				...
				y += lineSpacing;
				glRasterPos2i(x, y);
				line = "Time: " + int2str(int(realClock.ElapsedTime()))+"s";
				glutBitmapString(fontface,line.c_str());
				y += lineSpacing;
				glRasterPos2i(x, y);
				...
				y += lineSpacing;
				glRasterPos2i(x, y);
				int rem = int(5.0 - (realClock.ElapsedTime() - finishTime));
	...
	virtual void DoAltDrag(int dx, int dy, int button) {
		if (button == GLUT_LEFT_BUTTON) {
			...
			if (currentStep==3 && scenarioNumber == 0) {
				currentStep++;
				tutorialFinishTime = realClock.ElapsedTime();
			}
  ...
  void CheckCollisions()
  {
    ...
    //Check joint limits
    sim.odesim.robot(0)->GetConfig(robot->q);
    bool inJointLimits = true;
    for(int i=0;i<robot->q.n;i++)
      if(robot->q(i) < robot->qMin(i)-1e-3 || robot->q(i) > robot->qMax(i)+1e-3) {
	printf("Joint %d: %g not in [%g,%g]\n",i,robot->q(i),robot->qMin(i),robot->qMax(i));
	inJointLimits=false;
	break;
      }
    if(!inJointLimits) {
  ...
  void CheckCompletion()
  {
	...
	LogStatus("Finished");
	finished = true;
	finishTime = realClock.ElapsedTime();
      }
    }
...
int main(int argc, char** argv) {
	...
	*/
	...
	program.width = 1200;
	program.height = 700;
	return program.Run("Assisted Teleoperation GUI");
}
