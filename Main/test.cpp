...
public:
  ...
  virtual void RenderWorld()
  {
	...
	if(robot->geometry[j].tris.empty()) continue;
	Box3D bbox;
	GetBB(robot->geometry[j],bbox);
	Matrix4 basis;
	bbox.getBasis(basis);
	glColor3f(1,0,0);
  ...
  void Handle_Motion(int x, int y)
  {
    ...
    if(hoverWidget >= 0) {
      ...
      if(hoverWidget >= 0) poseWidgets[hoverWidget].SetHighlight(false);
      hoverWidget = -1;
