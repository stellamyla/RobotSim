...
public:
  ...
  Config tempq;
  int drawBBs,drawPoser,drawDesired,drawEstimated,drawContacts,drawWrenches,drawSensorPlot;
  ...
  virtual bool Initialize()
  {
    ...
    map<string,string> settings = sim.robotControllers[0]->Settings();
    ...
    */
    ...
    sensorPlot.x = 20;
    sensorPlot.y = 20;
    sensorPlot.width = 300;
    sensorPlot.height = 200;
    sensorPlot.autoXRange = false;
    sensorPlot.xmin = sim.time-2;
  ...
  virtual void RenderWorld()
  {
	...
	glPushMatrix();
	glMultMatrix(Matrix4(hoverRobot->robot->links[j].T_World));
	float color[4] = {1,1,0,0.5};
	glMaterialfv(GL_FRONT,GL_AMBIENT_AND_DIFFUSE,color); 
	hoverRobot->view.DrawLink_Local(j);
	glPopMatrix();
      }
	...
	for(size_t j=0;j<world->robots[i].robot->geometry.size();j++) {
	  if(world->robots[i].robot->geometry[j].tris.empty()) continue;
	  Box3D bbox;
	  GetBB(world->robots[i].robot->geometry[j],bbox);
	  Matrix4 basis;
	  bbox.getBasis(basis);
	  glColor3f(1,0,0);
      ...
      for(size_t i=0;i<world->rigidObjects.size();i++) {
	Box3D bbox;
	GetBB(world->rigidObjects[i].object->mesh,bbox);
	Matrix4 basis;
	bbox.getBasis(basis);
	glColor3f(1,0,0);
	drawOrientedWireBox(bbox.dims.x,bbox.dims.y,bbox.dims.z,basis);
      ...
      for(size_t i=0;i<world->terrains.size();i++) {
	Box3D bbox;
	GetBB(world->terrains[i].terrain->mesh,bbox);
	Matrix4 basis;
	bbox.getBasis(basis);
	glColor3f(1,0.5,0);
	drawOrientedWireBox(bbox.dims.x,bbox.dims.y,bbox.dims.z,basis);
  ...
  bool LoadMultiPath(const char* fn,bool constrainedInterpolate=true,Real interpolateTolerance=1e-2,Real durationScale=1.0)
  {
	...
	printf("Using existing timing in MultiPath\n");
    }
