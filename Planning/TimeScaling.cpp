...
bool TimeScalingSLP::AddVel2Bounds(int i,const Vector& Ai,const Vector& An,const Vector& bi)
{
  ...
  //DEBUG: print out some bounds on i=100
  //if(!GetActiveBounds(l,u,Ai,An,bi,nonredundant,(i==100))) {
...
bool TimeScalingSLP::Solve(int& maxIters,Real xtol,Real ftol)
{
  ...
  if(!feasible) {
	...
	if(d > lp.p(i)+Epsilon || d < lp.q(i)) {
	  printf("Constraint %d volation: %g <= %g <= %g\n",i,lp.q(i),d,lp.p(i));
	  feasible = false;
	}
      }
...
//initialize the solver with a better initial state
			       ...
			       Real ds0,Real dsEnd)
{
  ...
  return SolveMinTime(vmin,vmax,amin,amax,paramdivs,dxs,dxmins,dxmaxs,ddxmins,ddxmaxs,ds0,dsEnd);
}
					...
					Real ds0,Real dsEnd)
{
  ...
  return SolveMinTimeArcLength(vmin,vmax,amin,amax,paramdivs,dxs,dxmins,dxmaxs,ddxmins,ddxmaxs,ds0,dsEnd);
}
...
//these use a more detailed velocity bound (hexagon/parallelogram) rather than a box
#define NEW_VELOCITY_BOUNDS 0
//debug
			       ...
			       Real ds0,Real dsEnd)
{
  ...
  if(NEW_VELOCITY_BOUNDS) {
    for(int i=0;i<=n;i++) {
      Real dsmin=Inf;
      for(int j=0;j<d;j++)
	dsmin = Min(dsmin,Max(vmax[j]/dxs[i][j],vmin[j]/dxs[i][j]));
      slp.SetVelBound(i,dsmin);
    }
  }
  else {
  ...
  if(NEW_VELOCITY_BOUNDS) {
	...
	Real dx1 = dxs[i][j];
	Real dx2 = dxs[i+1][j];
	Assert(dbnd.x <= dx1 && dx1 <= dbnd.y);
	Assert(dbnd.x <= dx2 && dx2 <= dbnd.y);
	Real u1low = (dbnd.x - dx1)/abnd.x;
	Real u2low = 1.0 - (dbnd.y - dx2)/abnd.y;
	//dx1 - dx2 + abnd.y  = u * (abnd.y - abnd.x)
	Real intlow = (dx1-dx2+abnd.y)/(abnd.y-abnd.x);
	Real intvlow = dx1 + intlow*abnd.x;
	Real u1high = (dbnd.y - dx1)/abnd.y;
	Real u2high = 1.0 - (dbnd.x - dx2)/abnd.x;
	Real inthigh = (dx1-dx2+abnd.x)/(abnd.x-abnd.y);
	Real intvhigh = dx1 + inthigh*abnd.y;
	vector<Vector2> lowpoints,highpoints;
	if(intlow > 0 && intlow < 1.0 && intvlow > dbnd.x)
	  lowpoints.push_back(Vector2(intlow,intvlow));
	else {
	  if(u1low > 0 && u1low < 1.0) 
	    lowpoints.push_back(Vector2(u1low,dbnd.x));
	  if(u2low > 0 && u2low < 1.0) 
	    lowpoints.push_back(Vector2(u2low,dbnd.x));
	}
	if(inthigh > 0 && inthigh < 1.0 && intvhigh > dbnd.y)
	  highpoints.push_back(Vector2(inthigh,intvhigh));
	else {
	  if(u1high > 0 && u1high < 1.0) 
	    highpoints.push_back(Vector2(u1high,dbnd.y));
	  if(u2high > 0 && u2high < 1.0) 
	    highpoints.push_back(Vector2(u2high,dbnd.y));
	}
	//TODO: for edges that cross from negative to positive or vice versa, bound the segments
	//NOTE: lower bound on quadratic equation au^2 + bu + c is min(0,b/2,a+b)
	//ax^2 + by^2 <= c^2/(a+b)   =>   ax+by <= c
	if(dbnd.x < 0) {
	  for(size_t k=0;k<lowpoints.size();k++) {
	    if(lowpoints[k].y < 0) {
	      slp.AddVel2Bound(i,(1.0-lowpoints[k].x),lowpoints[k].x,Sqr(vmin[j]/lowpoints[k].y));
	    }
	  }
	}
	if(dbnd.y > 0) {
	  for(size_t k=0;k<highpoints.size();k++) {
	    if(highpoints[k].y > 0) {
	      slp.AddVel2Bound(i,(1.0-highpoints[k].x),highpoints[k].x,Sqr(vmax[j]/highpoints[k].y));
	    }
	  }
	}
      }
    }
    printf("New velocity bounds added %d constraints\n",slp.lp.A.m);
  }
			 ...
			 TimeScaling& scaling)
{
  ...
  for(size_t i=0;i<n;i++) {
    ...
    path.segments[i].Accel(1,amaxs[i]);
    /*
    for(int j=0;j<vmins[i].n;j++)
      if(vmins[i](j) > vmaxs[i][j]) Swap(vmins[i](j),vmaxs[i](j));
    for(int j=0;j<amins[i].n;j++)
      if(amins[i](j) > amaxs[i][j]) Swap(amins[i](j),amaxs[i](j));
    */
    vmaxs[i] = vmins[i];
    amaxs[i] = amins[i];
    /*
    //take collocation points at midpoints
    path.segments[i].Deriv(0.5,vmaxs[i]);
    vmaxs[i] = vmins[i];
    path.segments[i].Accel(0.5,amins[i]);
    amaxs[i] = amins[i];
    */
  ...
  //vs[n] = 3.0*(path.segments.back().x3-path.segments.back().x2);
  bool res=scaling.SolveMinTime(vmin,vmax,amin,amax,divs,vs,vmins,vmaxs,amins,amaxs,0.0,0.0);
...
ContactTimeScaling::ContactTimeScaling(Robot& robot)
  :cspace(robot),manifold(robot),torqueRobustness(0),frictionRobustness(0),forceRobustness(0)
{
}
...
bool ContactTimeScaling::SetParams(const MultiPath& path,const vector<Real>& paramDivs,int numFCEdges)
{
  ...
  vector<vector<GeneralizedCubicBezierCurve> > smoothPaths(path.sections.size());
  printf("Saving control points to cps.txt\n");
  ofstream out("cps.txt");
  for(size_t i=0;i<smoothPaths.size();i++) {
    Assert(!path.HasTiming(i));
    ...
    for(size_t j=0;j<smoothPaths[i].size();j++) {
      out<<smoothPaths[i][j].x0<<endl;
      out<<smoothPaths[i][j].x1<<endl;
      out<<smoothPaths[i][j].x2<<endl;
      out<<smoothPaths[i][j].x3<<endl;
    }
  }
  out.close();
  ...
  for(size_t i=0;i<smoothPaths.size();i++) {
    Real totalDuration = 1.0/smoothPaths.size();
    traj.path.segments.insert(traj.path.segments.end(),smoothPaths[i].begin(),smoothPaths[i].end());
    Real dt = totalDuration / smoothPaths[i].size();
    traj.path.durations.resize(traj.path.segments.size(),dt);
  }
  ...
  for(size_t i=0;i<paramDivs.size();i++) {
    Real duration,edgeparam;
    int seg=paramSections[i];
    Assert(seg >= 0 && seg < (int)path.sections.size());
    Real sectionParam = paramDivs[i]*path.sections.size()-seg;
    duration = 1.0/(path.sections.size()*(path.sections[seg].milestones.size()-1));
    int edge = (int)Floor(sectionParam*(path.sections[seg].milestones.size()-1));
    edgeparam = sectionParam*(path.sections[seg].milestones.size()-1)-edge;
    if(edge >= (int)path.sections[seg].milestones.size()-1) {
      edge--;
      edgeparam = 1;
    }
    Assert(edge >= 0 && edge < (int)path.sections[seg].milestones.size()-1);
    const GeneralizedCubicBezierCurve& curve = smoothPaths[seg][edge];
    curve.Eval(edgeparam,xs[i]);
    curve.Deriv(edgeparam,dxs[i]);
    curve.Accel(edgeparam,ddxs[i]);
    if(duration != 0) {
      dxs[i] /= duration;
      ddxs[i] /= Sqr(duration);
    }
    else {
      printf("Warning, path segment %d has duration 0\n",i);
    }
  }
  ...
  for(size_t i=0;i<paramDivs.size();i++) {
    ...
    //|a dds + b ds^2 + c - Jtf| <= torquemax*(1-robustnessFactor)
    for(int j=0;j<a.n;j++) {
      lp.A(j,0) = b(j);
      lp.A(j,1) = a(j);
      lp.p(j) = robot.torqueMax(j)*(1.0-torqueRobustness)-c(j);
      lp.q(j) = -robot.torqueMax(j)*(1.0-torqueRobustness)-c(j);
    }
	...
	const Vector3& p=formation.contacts[l][j].x;
	Vector3 v;
	int k=link;
	while(k!=-1) {
...
bool ContactTimeScaling::Optimize()
{
  TimeScalingSLP slp(paramDivs);
  slp.SetFixed(0,0.0);
  slp.SetFixed(paramDivs.size()-1,0.0);
  for(size_t i=0;i+1<paramDivs.size();i++) {
    Real scale = 0.5/(paramDivs[i+1]-paramDivs[i]);
    for(size_t j=0;j<ds2ddsConstraintNormals[i].size();j++) {
      Real ai = ds2ddsConstraintNormals[i][j].x - ds2ddsConstraintNormals[i][j].y*scale;
      Real an = ds2ddsConstraintNormals[i][j].y*scale;
      Real b = ds2ddsConstraintOffsets[i][j];
      slp.AddVel2Bound(i,ai,an,b);
      /*
      if(i == 0)
	printf("%g * x[0] + %g * x[1] <= %g\n",ai,an,b);
      if(i == 1)
	printf("%g * x[1] + %g * x[2] <= %g\n",ai,an,b);
      */
    }
  }
  //getchar();
  ...
  if(!res) {
    printf("Unable to solve for time scaling\n");
    return false;
  }
...
bool ContactTimeScaling::Check(const MultiPath& path)
{
  ...
  for(size_t i=0;i<paramDivs.size();i++) {
    ...
    for(int j=0;j<solver.t.n;j++) {
      if(Abs(solver.t(j)) > robot.torqueMax(j)) {
  ...
  //test off the colocation points
  for(size_t i=0;i+1<paramDivs.size();i++) {
    int seg=paramSections[i];
    Real t=(traj.timeScaling.times[i]+traj.timeScaling.times[i+1])*0.5;
    traj.Eval(t,x);
    traj.Deriv(t,dx);
    traj.Accel(t,ddx);
    ...
    bool res=solver.Solve();
    if(!res) {
      printf("TorqueSolver was not able to compute a solution at midpoint param %d (time %g/%g)\n",i,t,traj.timeScaling.times.back());
      return false;
    }
    for(int j=0;j<solver.t.n;j++) {
      if(Abs(solver.t(j)) > robot.torqueMax(j)) {
