...
{
  vector<Real> collisionMargin;  //margin of collision avoidance
...
struct ObjectPlannerSettings
{
  ...
  Real collisionMargin;    //margin of collision avoidance
  Real collisionEpsilon;   //threshold for edge feasibility checks
...
struct WorldPlannerSettings
{
  ...
  bool CheckCollision(RobotWorld& world,Geometry::CollisionMesh& mesh,int id,Real tol=0);
  Real DistanceLowerBound(RobotWorld& world,int id1,int id2=-1,Real eps=0,Real bound=Inf);
  Real DistanceLowerBound(RobotWorld& world,Geometry::CollisionMesh& mesh,int id,Real eps=0,Real bound=Inf);
  ...
  void EnumerateCollisionQueries(RobotWorld& world,int id1,int id2,
				 ...
				 vector<Geometry::CollisionMeshQueryEnhanced>& queries);
  void EnumerateCollisionQueries(RobotWorld& world,Geometry::CollisionMesh& mesh,int id,
