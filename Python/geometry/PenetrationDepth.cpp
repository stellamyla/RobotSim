...
void ApproximatePenetrationDepth::ComputeInitial(const RigidTransform& f1,const RigidTransform& f2,const int tc1[], const int tc2[],int n)
{
  ...
  for(int i=0;i<n;i++) {
    ...
    T2.c = f2*T2.c;
    if(!T1.intersect(T2)) {
      cout<<"Error, T1 doesn't intersect T2!"<<endl;
      cout<<"i="<<i<<endl;
  ...
  */
  ...
  for(int i=0;i<n;i++) {
    ...
    */
    ...
    if(Max(da,Max(db,dc)) < Zero || Min(Min(da,db),dc) > Zero) {
      //cout<<"Warning: triangle "<<tc1[i]<<" does not intersect "<<tc2[i]<<endl;
      continue;
    }
...
#if VERBOSE
  if(!collisions.empty())
    cout<<collisions.size()<<" edge collisions."<<endl;
#endif
  ...
  for(Iterator i=collisions.begin();i!=collisions.end();i++) {
	...
	if(j->second.voutside == i->second.vinside) break;
      if(j != range.second) {
	//cout<<"Looks like an edge penetration..."<<endl;
	...
	if(p1.distance(a) > Epsilon) {
	  cerr<<"Warning: distance from triangle to inside point is greater than 0! "<<p1.distance(a)<<endl;
	  getchar();
	}
	if(p2.distance(b) > Epsilon) {
	  cerr<<"Warning: distance from triangle to inside point is greater than 0! "<<p2.distance(b)<<endl;
	  getchar();
	}
	...
	//cout<<"Hmm... the outside vertex is a fringe vertex, what do we do?"<<endl;
      }
    }
  ...
  for(Iterator i=collisions.begin();i!=collisions.end();) {
	...
	cout<<"PenetrationDepth::ComputeInitial(): Warning, initial point has a negative depth"<<endl;
	//getchar();
      }
...
#if VERBOSE
  if(maxDepth != Zero)
    cout<<"MaxDepth "<<maxDepth<<", "<<visitedNodes<<" of "<<m1.verts.size()<<endl;
#endif
}
