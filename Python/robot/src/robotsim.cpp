...
void GetMesh(const Geometry::CollisionMesh& mesh,TriangleMesh& tmesh)
{
...
void GetMesh(const TriangleMesh& tmesh,Geometry::CollisionMesh& mesh)
{
...
{
  if(object < 0  || object >= worlds[index]->world.rigidObjects.size())
...
{
  if(terrain < 0  || terrain >= worlds[index]->world.terrains.size())
...
TriangleMesh RobotModelLink::getMesh()
{
  TriangleMesh mesh;
  GetMesh(robot->geometry[index],mesh);
  return mesh;
}

void RobotModelLink::setMesh(const TriangleMesh& mesh)
{
  GetMesh(mesh,robot->geometry[index]);
}
...
void RobotModelLink::drawLocalGL(bool keepAppearance)
{
  RobotWorld& world = worlds[this->world]->world;
  if(keepAppearance) {
    const GLColor& color = world.robots[robotIndex].view.colors[index];
    glMaterialfv(GL_FRONT,GL_AMBIENT_AND_DIFFUSE,color.rgba); 
    color.setCurrentGL(); //in case the link is drawn w/o lighting?
  }
...
TriangleMesh RigidObjectModel::getMesh()
{
  TriangleMesh mesh;
  GetMesh(object->mesh,mesh);
  return mesh;
}

void RigidObjectModel::setMesh(const TriangleMesh& mesh)
{
  GetMesh(mesh,object->mesh);
}
...
TriangleMesh TerrainModel::getMesh()
{
  TriangleMesh mesh;
  GetMesh(terrain->mesh,mesh);
  return mesh;
}

void TerrainModel::setMesh(const TriangleMesh& mesh)
{
  GetMesh(mesh,terrain->mesh);
}
...
SimBody Simulator::getBody(const RobotModelLink& link)
{
  ...
  body.mesh = sim->odesim.robot(link.robotIndex)->triMesh(link.index);
  return body;
}
...
SimBody Simulator::getBody(const RigidObjectModel& object)
{
  ...
  body.mesh = sim->odesim.object(object.index)->triMesh();
  return body;  
}
...
SimBody Simulator::getBody(const TerrainModel& terrain)
{
  SimBody body;
  body.body = NULL;
  body.mesh = sim->odesim.envMesh(terrain.index);
  return body;
}
