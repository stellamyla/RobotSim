...
{
  out<<h.link<<endl;
  out<<h.contacts.size()<<endl;
  for(size_t i=0;i<h.contacts.size();i++) {
    out<<h.contacts[i].x<<"\t\t"<<h.contacts[i].n<<"\t\t"<<h.contacts[i].kFriction<<endl;
  }
  out<<"position"<<endl;
  out<<"  "<<h.ikConstraint.localPosition<<endl;
  out<<"  "<<h.ikConstraint.endPosition<<endl;
  if(h.ikConstraint.numRotTerms==1) {
    out<<"axis"<<endl;
    out<<"  "<<h.ikConstraint.localAxis<<endl;
    out<<"  "<<h.ikConstraint.endRotation<<endl;
  }
  else if(h.ikConstraint.numRotTerms == 3) {
    out<<"rotation"<<endl;
    out<<"  "<<h.ikConstraint.endRotation<<endl;
  }
  out<<"end"<<endl;
...
istream& operator >> (istream& in, Hold& h)
{
  ...
  in>>h.link;
  in>>numContacts;
  Assert(0<numContacts && numContacts < 100);  //increase this limit if necessary?
  if(in.bad()) return in;
  h.contacts.resize(numContacts);
  for(int i=0;i<numContacts;i++) {
    in>>h.contacts[i].x;
    in>>h.contacts[i].n;
    in>>h.contacts[i].kFriction;
  }
  if(in.bad()) return in;
  ...
  if(str != "end") {  //this is the default
    Vector3 localPos0,rot;
    istringstream ss; ss.str(str);
    ss >> localPos0.x;  in>>localPos0.y;  in>>localPos0.z;
    in>>rot;
    h.SetupIKConstraint(localPos0,rot);
  }
  return in;
