...
void JointPositionSensor::Simulate(ControlledRobotSimulator* robot)
{
  ...
  if(!indices.empty()) {
    //only read a subset
    Vector qread(q.n,Zero);
    for(size_t i=0;i<indices.size();i++)
      qread(indices[i]) = q(indices[i]);
    swap(qread,q);
  }
}
...
//defined in Modeling/MultiPath.cpp
void GetAttributeMap(TiXmlElement* node,map<string,string>& attrMap);
void SetAttributeMap(TiXmlElement* node,const map<string,string>& attrMap);
...
void RobotSensors::SaveSettings(TiXmlElement* node)
{
  ...
  for(size_t i=0;i<sensors.size();i++) {
    ...
    map<string,string> settings = sensors[i]->Settings();
    SetAttributeMap(&c,settings);
