...
void TestShortcutting(SingleRobotCSpace* cspace,MotionPlannerFactory& plannerFactory,const Config& a,const Config& b)
{
  ...
  for(int p = 0; p < 10; p++) {
    ...
    //TEST
    printf("Checking ramp 0\n");
    for(size_t i=0;i<path.ramps.size();i++) {
      timer.Reset();
      res=CheckRamp(path.ramps[i],cspace,tol);
      printf("Approx (res=%d, time=%g)\n",(int)res,timer.ElapsedTime());
      timer.Reset();
      res=CheckRampExact(path.ramps[i],cspace,1000);
      printf("Exact (res=%d, time=%g)\n",(int)res,timer.ElapsedTime());
    }
    getchar();
    ...
    for(int i=0;i<10;i++) {
      timer.Reset();
      //ns += path.Shortcut(cspace,tol,Inf,maxShortcutIters/10);
      ns += path.Shortcut(cspace,-1,Inf,maxShortcutIters/10);
    ...
    timer.Reset();
    for(size_t i=0;i<path.ramps.size();i++) {
      if(!CheckRamp(path.ramps[i],cspace,tol)) {
    ...
    */
    ...
    /* for(int trial = 0;trial < 5; trial++)*/ {
      path = porig;
      timer.Reset();
      ns = path.DynamicShortcut(0.0,0.01,cspace,tol);
...
void TestDynamicShortcutting(SingleRobotCSpace& freeSpace,const ParabolicRamp::DynamicPath& porig)
{
  ...
  int ns=0;
  timer.Reset();
  ns = path.DynamicShortcut(0.1,0.01,&freeSpace,tol);
  printf("Dynamic shortcutting with window %g made %d shortcuts, took %g seconds\n",0.1,ns,timer.ElapsedTime());

  path = porig;
  timer.Reset();
  ns=path.DynamicShortcut(0.5,0.01,&freeSpace,tol);
  printf("Dynamic shortcutting with window %g made %d shortcuts, took %g seconds\n",0.5,ns,timer.ElapsedTime());

  path = porig;
  timer.Reset();
  ns=path.DynamicShortcut(1.0,0.01,&freeSpace,tol);
  printf("Dynamic shortcutting with window %g made %d shortcuts, took %g seconds\n",1.0,ns,timer.ElapsedTime());

  path = porig;
  timer.Reset();
  ns=path.DynamicShortcut(2.0,0.01,&freeSpace,tol);
