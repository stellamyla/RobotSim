...
bool Robot::LoadRob(const char* fn) {
	...
	printf("Done reading robot file, %d links read\n", n);
	if (parents.empty()) {
		parents.resize(n);
	...
	if(geomTransformIndex.size() != geomTransform.size()){
		printf("the size of geomTransformIndex and geomTransform size not match!");
		return false;
	}
	...
	for (size_t i = 0; i < noCollision.size(); i++) {
		if (noCollision[i] >= (int) links.size()) {
			printf("Invalid no-collision index %d\n", noCollision[i]);
			return false;
		}
...
//		Assert(joints.size() == links.size());
	...
	for (size_t i = 0; i < mountLinks.size(); i++) {
	  mountFiles[i] = fnPath + mountFiles[i];
		...
		if (Meshing::CanLoadTriMeshExt(FileExtension(mountFiles[i].c_str()))) {
			...
			if(!Import(mountFiles[i].c_str(),mesh)) {
				fprintf(stderr, "Error loading mount geometry file %s\n",
						mountFiles[i].c_str());
				return false;
			}
			Mount(mountLinks[i], mesh, mountT[i]);
		} else {
			Robot subchain;
	...
	printf("Initializing self-collisions\n");
	...
	if (selfCollision.empty()) {
		...
		for (size_t i = 0; i < selfCollision.size(); i++) {
			if (selfCollision[i].first >= (int) links.size()
				...
				printf("Invalid collision index %d-%d\n",
...
bool Robot::SaveGeometry(const vector<string>& geomFiles) {
	for (size_t i = 0; i < links.size(); i++) {
		if (!geometry[i].tris.empty()) {
			ofstream out(geomFiles[i].c_str(), ios::out);
			if (!out.is_open()) {
				cerr << "Unable to open geometry file " << geomFiles[i] << endl;
				return false;
			}
			out << geometry[i] << endl;
			out.close();
...
void Robot::Mount(int link, const Meshing::TriMesh& mesh,
		const RigidTransform& T) {
	vector<Meshing::TriMesh> mergeMeshes(2);
...
void Robot::Mount(int link, const Robot& subchain, const RigidTransform& T) {
	...
	for (size_t j = 0; j < subchain.links.size(); j++) {
		if (subchain.parents[j] < 0 && !geometry[link].tris.empty()) {
...
void Robot::ComputeLipschitzMatrix() {
	...
	for (size_t i = 0; i < links.size(); i++) {
		...
		RigidTransform temp;
		temp.setIdentity();
		swap(geometry[i].currentTransform, temp);
		GetBB(geometry[i], b);
		swap(geometry[i].currentTransform, temp);
		s.center = b.origin + 0.5 * b.dims.x * b.xbasis
				+ 0.5 * b.dims.y * b.ybasis + 0.5 * b.dims.z * b.zbasis;
		s.radius = 0;
		for (size_t j = 0; j < geometry[i].verts.size(); j++)
			s.radius = Max(s.radius, geometry[i].verts[j].norm());
