...
void RandomizedSelfCollisionDistances(RobotWithGeometry& robot,Array2D<Real>& minDistance,Array2D<Real>& maxDistance,int numSamples)
{
  ...
  Array2D<SmartPointer<CollisionMeshQuery> > queries(robot.q.n,robot.q.n);
  for(int i=0;i<robot.q.n;i++) {
    for(int j=i+1;j<robot.q.n;j++) {
      if(!robot.geometry[i].tris.empty() && robot.geometry[j].tris.empty())
	queries(i,j) = new CollisionMeshQuery(robot.geometry[i],robot.geometry[j]);
    }
  }
