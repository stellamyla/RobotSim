...
bool XmlRobot::GetRobot(Robot& robot)
{
  ...
  Vector q;
  if(e->QueryValueAttribute("q",&q)==TIXML_SUCCESS || e->QueryValueAttribute("config",&q)==TIXML_SUCCESS) {
    if(q.n != robot.q.n) {
...
bool XmlRigidObject::GetObject(RigidObject& obj)
{
  ...
  if(geom) {
    ...
    obj.mesh.InitCollisions();
  }
  if(obj.mesh.tris.empty()) {
...
bool XmlTerrain::GetTerrain(Environment& env)
{
  ...
  if(transform) {
    ...
    env.mesh.Transform(xform);
    env.mesh.InitCollisions();
  }
  return true;
}
 ...
 public:
  ...
  bool GetView(ViewEnvironment& view)
  {
    if(e->Attribute("color")) {
      ...
      view.faceColor.set(rgb.x,rgb.y,rgb.z,a);
    }
...
{
  if(!doc.RootElement()) return NULL;
